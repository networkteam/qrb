...
Some clauses require a bare column name, e.g. the conflict target of `ON CONFLICT` or the column in `SET`.
Use `Unqualified()` (or `UnqualifiedString()` for a raw string) to strip the qualification from a table-qualified
identifier, e.g. one generated by [construct](https://github.com/networkteam/construct):

```go
userEmail := N("users.email") // e.g. generated by construct

q := InsertInto(N("users")).
    ColumnNames("email", "name").
    Values(String("john@example.com"), String("John")).
    OnConflict(userEmail.Unqualified()).DoNothing()
```
